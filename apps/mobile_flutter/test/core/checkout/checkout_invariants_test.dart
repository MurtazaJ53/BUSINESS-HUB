...
void main() {
  group('Checkout Financial Invariants', () {
    test('Invariant: totalCollected + due == total for all positive tenders', () {
      final billTotals = [100.0, 250.0, 499.0, 1000.0, 15750.0];
      final tenderedAmounts = [0.0, 50.0, 100.0, 250.0, 500.0, 1000.0, 20000.0];
      ...
      for (final total in billTotals) {
        for (final tender in tenderedAmounts) {
          ...
          expect(
            ...
            reason: 'totalCollected exceeded bill for bill=$total, tender=$tender',
          );
    ...
    test('Invariant: Multi-mode tender split conservation', () {
      ...
      for (final upi in upiAmounts) {
        for (final cash in cashAmounts) {
          ...
          if (upi > 0) lines.add(CheckoutPaymentEntry(mode: 'UPI', amount: upi));
          if (cash > 0) lines.add(CheckoutPaymentEntry(mode: 'CASH', amount: cash));
