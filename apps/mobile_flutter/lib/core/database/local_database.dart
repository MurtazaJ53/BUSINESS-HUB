...
class InventoryPrivateEntries extends Table {
  ...
  TextColumn get id => text()();
  RealColumn get costPrice => real().named('cost_price').withDefault(const Constant(0))();
  TextColumn get supplierId => text().named('supplier_id').nullable()();
  TextColumn get lastPurchaseDate => text().named('last_purchase_date').nullable()();
  IntColumn get updatedAt => integer().named('updated_at').withDefault(const Constant(0))();
  BoolColumn get tombstone => boolean().withDefault(const Constant(false))();
}
...
class SalesEntries extends Table {
  ...
  TextColumn get discountType => text().named('discount_type').withDefault(const Constant('fixed'))();
  TextColumn get paymentMode => text().named('payment_mode').withDefault(const Constant('CASH'))();
