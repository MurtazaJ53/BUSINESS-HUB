...
export async function GET() {
  try {
    ...
    const [listRes, summaryRes] = await Promise.all([
      fetch(`${API_BASE_URL}/shops/${shopId}/purchases/`, { headers, cache: "no-store" }),
      fetch(`${API_BASE_URL}/shops/${shopId}/purchases/summary/`, { headers, cache: "no-store" }),
    ]);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${API_BASE_URL}/shops/${shopId}/purchases/`, {
      method: "POST",
      headers: {
